...
\begin{exemple}
  ...
  $f: \RR^p \rightarrow \zo$ peut être obtenu en seuillant une fonction de
...
intensif en temps de calcul d'y trouver un bon modèle.
...
est équivalente à l'\textbf{entropie croisée}, définie pour $\YY = \zo$.
...
euclidienne, apprend un modèle non paramétrique : il ne s'agit pas d'écrire la
fonction de décision comme une expression explicite des variables prédictives
et d'apprendre les paramètres de cette expression.
...
uniquement. Nous traiterons de classification paramétrique dans la PC~5.
