...
Nous supposons dans ce cours que nos données sont \textbf{structurées},
c'est-à-dire présentées sous forme vectorielle. Ce n'est pas le cas de nombreux
types de données telles que du texte, des images, du son, des séquences d'ADN, ou des
...
\begin{itemize}
\item éliminer, par exemple à l'aide d'un test du chi2 comme vu dans la PC~1,
...
\[
  \Sigma_{jk} = \frac1n \sum_{i=1}^n (x_j^i - \overline{x_j})  (x_k^i - \overline{x_k}) = \frac1n \sum_{i=1}^n x_j^i x_k^i
   = \langle \xx_j, \xx_k \rangle
 \]
 et $\Sigma = \frac1n X^\top X$ est une matrice symétrique avec des 1 sur la diagonale (car $\Sigma_{jj} = \text{var}(\xx_j) = 1$).
