...
\begin{figure}[h]
  ...
  \end{subfigure}
  \caption{Illustration des concepts de biais et de variance par analogie avec un jeu de fléchettes. La quantité à estimer est le centre de la cible ; les fléchettes sont les estimateurs.}
...
On dit que l'estimateur $\Theta_n$ de $\theta$ \textbf{est convergent} s'il converge en probabilité vers $\theta:$
\begin{equation}
...
\[
  \EE((\Theta_n - \theta)^2) = \VV(\Theta_n - \theta) + [\EE(\Theta_n) - \theta]^2,
\]
...
% \erratum{Voir fichier erratum\_estimation.pdf pour une définition correcte de la
%   vraisemblance dans le cas d'une variable aléatoire à densité.}
...
Notez que la loi de l'échantillon aléatoire $(X_1, X_2, \dots, X_n)$ est donnée par
$  \PP_{X_1, X_2, \dots, X_n ; t} = \prod_{i=1}^n \PP_{X; t} $
car les $X_i$ sont indépendantes et identiquement distribuées.

% \begin{itemize}
% \item Si $X$ est discrète, on appelle $p_\theta : \NN \rightarrow [0, 1]$ la
%   fonction de masse de $X$ ( $\PP(X=k) = p_\theta(k)$), et la
%   \textbf{vraisemblance} de l'échantillon $(x_1, x_2, \dots, x_n)$ est donnée
%   par
%   \[
%     L(x_1, x_2, \dots, x_n; \theta) = \PP(X_1=x_1, X_2=x_2, \dots, X_n=x_n) = \prod_{i=1}^n  p_\theta(x_i).
%   \]
%   La décomposition en produit indexé par $i=1, \dots, n$ vient de
%   l'indépendance des éléments de l'échantillon aléatoire.
% \item Si $X$ est continue, on pose $f_\theta : \RR \rightarrow \RR$ sa densité,
%   et la \textbf{vraisemblance} de l'échantillon $(x_1, x_2, \dots, x_n)$ est
%   donnée de manière analogue par
%   \[
...
\begin{exemple}
  ...
  \begin{align*}
    \hatmle{p} & = \argmax_{p \in [0, 1]} \sum_{i=1}^n \ln \PP_{X; p}(X=x_i)
  ...
  \begin{equation*}
    \frac{d L}{d p} = \sum_{i=1}^n x_i \frac{1}{p} - 
...
En particulier, si $\Theta$ est discrète, la formule de Bayes nous permet
...
% $(X_1, X_2, \dots, X_n)$.
% Il s'agit ici de maximiser cette espérance pour toutes les valeurs possibles de
% l'échantillon aléatoire, donc en conditionnant par rapport à
% $X_1, X_2, \dots, X_n$.
...
\]
La fonction de $\RR_+$ dans $\RR$ qui à $s$ associe
$-n \ln(s) + \sum_{i=1}^n \ln(x_i) - \frac{1}{2s} \sum_{i=1}^n x_i^2$ est une
fonction concave en $s$. On obtient donc son maximum en annulant sa dérivée, qui vaut :
\[
